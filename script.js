...
// --- SKETCH DESIGNER STATE ---
let sketchInvoiceNo = '';
let sketchCustomerName = '';
let sketchCustomerPhone = '';

const SKETCH_PREMIUM_COLORS = [
    '#000000', '#4b5563', '#dc2626', '#ea580c', '#eab308',
    '#16a34a', '#2563eb', '#9333ea', '#ec4899', '#0d9488'
];
let sketchCurrentColor = '#000000';
let sketchCurrentStrokeWidth = 3;

let sketchCanvas = null;
let sketchCtx = null;
let sketchCurrentMode = 'select'; 

let sketchShapes = []; 
let sketchFreehandLines = []; 
let sketchCurrentLine = null; 

let selectedSketchShape = null;
let isSketchDrawing = false;
let isSketchDragging = false;
let isSketchRotating = false;
let isSketchResizing = false;

let sketchDragOffsetX = 0;
let sketchDragOffsetY = 0;
let sketchRotateCenter = { x: 0, y: 0 };
let sketchStartAngle = 0;
...
function saveToLocal(key, data) {
    try {
        ...
        console.warn('LocalStorage is not accessible. Using in-memory storage.', e);
...
async function unverifyInvoice(invoiceNo) {
    ...
    <button onclick="openSketchDesigner('${inv.InvoiceNo}')" 
            class="text-pink-600 hover:text-pink-900 transition duration-150" title="تصميم كروكي صالون">
        كروكي
    </button>

    ${(inv.SketchData && inv.SketchData.trim() !== '') || getFromLocal(`sketch_data_${inv.InvoiceNo}`) ? 
        `<button onclick="generatePrintableSketch('${inv.InvoiceNo}')" 
                class="text-red-600 hover:text-red-800 transition duration-150" title="طباعة الكروكي">
    ...
    tbody.innerHTML = rows;
        ...
        document.addEventListener('DOMContentLoaded', () => {
            // Clean up duplicate modals from index.html at runtime to avoid DOM conflicts
            const existingModals = document.querySelectorAll('#printable-sketch-modal');
            existingModals.forEach(el => el.remove());

            const existingInvoiceModals = document.querySelectorAll('#printable-invoice-modal');
            if (existingInvoiceModals.length > 1) {
                for (let i = 1; i < existingInvoiceModals.length; i++) {
                    existingInvoiceModals[i].remove();
                }
            }

            // Inject printable sketch modal dynamically & ensure modals are direct children of main-content
            const mainContentEl = document.getElementById('main-content');
            if (mainContentEl) {
                // Ensure printable invoice modal is a direct child of main-content
                const invoiceModal = document.getElementById('printable-invoice-modal');
                if (invoiceModal) {
                    mainContentEl.appendChild(invoiceModal);
                    // Add manual print button if it doesn't exist
                    const actionsDiv = invoiceModal.querySelector('.space-x-2.space-x-reverse');
                    if (actionsDiv && !actionsDiv.querySelector('[onclick="window.print()"]')) {
                        const printBtn = document.createElement('button');
                        printBtn.className = 'bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-6 rounded-xl shadow-md transition duration-150';
                        printBtn.innerHTML = '<i class="fas fa-print ml-2"></i> طباعة';
                        printBtn.setAttribute('onclick', 'window.print()');
                        actionsDiv.insertBefore(printBtn, actionsDiv.firstChild);
                    }
                }

                const sketchModalDiv = document.createElement('div');
                sketchModalDiv.id = 'printable-sketch-modal';
                sketchModalDiv.className = 'fixed inset-0 bg-gray-600 bg-opacity-75 hidden items-center justify-center p-4 z-50 print:bg-white print:p-0';
                sketchModalDiv.setAttribute('onclick', "closeModal('printable-sketch-modal')");
                sketchModalDiv.innerHTML = `
                    <div class="bg-white rounded-xl shadow-2xl p-6 w-full max-w-4xl modal-content-custom print:shadow-none print:p-0" onclick="event.stopPropagation()">
                        <div class="flex justify-between items-center mb-4 no-print">
                            <h3 class="text-2xl font-bold text-gray-800">طباعة الكروكي</h3>
                            <div class="space-x-2 space-x-reverse">
                                <button onclick="window.print()" class="bg-amber-500 hover:bg-amber-600 text-white font-bold py-2 px-6 rounded-xl shadow-md transition duration-150">
                                    <i class="fas fa-print ml-2"></i> طباعة
                                </button>
                                <button type="button" onclick="closeModal('printable-sketch-modal')" class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4 rounded-xl">إغلاق</button>
                            </div>
                        </div>
                        
                        <div id="sketch-print-content" class="invoice-box bg-white p-4">
                            <!-- Header -->
                            <div class="flex justify-between items-center border-b-2 border-black pb-4 mb-4" style="display: flex; justify-content: space-between; align-items: center; direction: rtl;">
                                <div class="flex items-center" style="display: flex; align-items: center;">
                                    <img id="print-sketch-logo" style="height: 80px;" alt="Logo" class="ml-3">
                                    <div class="text-right" style="text-align: right; margin-right: 10px;">
                                        <p class="text-2xl font-extrabold text-black">MEUBLE ART DESIGN</p>
                                        <p class="text-xs text-gray-500">حي النهضة رقم 135 بلوك M</p>
                                        <p class="text-xs text-gray-500">الهاتف: 0640442778</p>
                                    </div>
                                </div>
                                <div class="text-left text-sm space-y-1" style="direction: rtl; text-align: left;">
                                    <p><strong>الزبون:</strong> <span id="print-sketch-customer-name"></span></p>
                                    <p><strong>الهاتف:</strong> <span id="print-sketch-customer-phone"></span></p>
                                    <p><strong>فاتورة رقم:</strong> <span id="print-sketch-invoice-no" class="font-bold text-amber-600"></span></p>
                                    <p class="text-xs text-gray-400"><strong>التاريخ:</strong> <span id="print-sketch-date"></span></p>
                                </div>
                            </div>
                            
                            <!-- The Canvas for Sketch -->
                            <div class="flex justify-center items-center my-6 border border-gray-300 p-2" style="background-color: #fff; display: flex; justify-content: center; align-items: center;">
                                <canvas id="print-sketch-canvas" width="800" height="850" style="max-width: 100%; height: auto; display: block; border: 1px solid #d1d5db;"></canvas>
                            </div>
                        </div>
                    </div>
                `;
                mainContentEl.appendChild(sketchModalDiv);
            }
            ...
            // ربط أحداث الكروكي (Mouse and Touch event listeners for the sketch canvas)
            const canvasEl = document.getElementById('sketch-drawing-canvas');
            if (canvasEl) {
                canvasEl.addEventListener('mousedown', onSketchPointerStart);
                canvasEl.addEventListener('mousemove', onSketchPointerMove);
                canvasEl.addEventListener('mouseup', onSketchPointerEnd);

                canvasEl.addEventListener('touchstart', onSketchPointerStart, { passive: false });
                canvasEl.addEventListener('touchmove', onSketchPointerMove, { passive: false });
                canvasEl.addEventListener('touchend', onSketchPointerEnd, { passive: false });
            }
        });
            ...
            scriptRunHelper('doLogin', [username, password], (result) => {
                if (result && result.success) {
                    try {
                        ...
                        console.error('Error during post-login UI update:', e);
        ...
        function handleLogout() {
             removeFromLocal(StorageKeys.USER);
        ...
        function updateSketchCanvasTooltip(e) {
            if (!sketchCanvas) return;
            if (isSketchDrawing || isSketchDragging || isSketchRotating || isSketchResizing) return;
            const p = getSketchCanvasCoords(e);
            let found = null;
            for (let i = sketchShapes.length - 1; i >= 0; i--) {
                if (isPointInSketchShape(p, sketchShapes[i])) {
                    found = sketchShapes[i];
                    break;
                }
            }
            if (found) {
                const names = {
                    ...
                    'rect': 'مستطيل',
                    'circle': 'دائرة',
                    'roundrect': 'مستطيل بزوايا دائرية',
                    'roundrect_top': 'مستطيل مقوس من الأعلى',
                    'cut_corner_square': 'مربع مشطوب الزاوية',
                    'open_left_rect': 'مستطيل مفتوح اليسار',
                    'shelf': 'رف / لوح خشب',
                    'corner_salon': 'زاوية صالون / ركنة',
                    'text': 'نص',
                    'line': 'خط مستقيم'
                };
                let name = names[found.type] || 'شكل هندسي';
                if (found.type === 'text') {
                    name = `نص: "${found.text}"`;
                }
                sketchCanvas.title = name;
            } else {
                sketchCanvas.title = '';
            }
        }

        function openSketchDesigner(invoiceNo) {
            ...
            sketchInvoiceNo = invoice.InvoiceNo;
            sketchCustomerName = invoice.CustomerName || '';
            sketchCustomerPhone = invoice.CustomerPhone || '';

            // Populate UI Info
            document.getElementById('pdf-sketch-customer-name').textContent = sketchCustomerName;
            document.getElementById('pdf-sketch-customer-phone').textContent = sketchCustomerPhone;
            document.getElementById('pdf-sketch-invoice-no').textContent = sketchInvoiceNo;
            
            const options = { year: 'numeric', month: 'long', day: 'numeric' };
            document.getElementById('pdf-sketch-date').textContent = new Date().toLocaleDateString('ar-MA', options);

            // Initialize canvas
            sketchCanvas = document.getElementById('sketch-drawing-canvas');
            sketchCanvas.width = 734;
            sketchCanvas.height = 698;
            sketchCtx = sketchCanvas.getContext('2d');

            // Reset drawing data
            sketchShapes = [];
            sketchFreehandLines = [];
            sketchCurrentLine = null;
            selectedSketchShape = null;
            isSketchDrawing = false;
            isSketchDragging = false;
            isSketchRotating = false;
            isSketchResizing = false;
            ...
            // Auto load sketch data
            loadSketchFromServer();
            ...
            // Inject tooltips, inline SVGs and hide select/draw mode buttons
            setTimeout(() => {
                ...
                shapeButtons.forEach(btn => {
                    ...
                    if (onclickStr.includes('square') && !onclickStr.includes('cut_corner_square')) {
                        btn.title = "مربع";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="3" width="18" height="18" rx="0"/></svg>`;
                    } else if (onclickStr.includes('cut_corner_square')) {
                        btn.title = "مربع مشطوب الزاوية";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M 3 9 L 9 3 L 21 3 L 21 21 L 3 21 Z"/></svg>`;
                    } else if (onclickStr.includes('roundrect_top')) {
                        btn.title = "مستطيل مقوس من الأعلى";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M3 20V10a7 7 0 0 1 7-7h4a7 7 0 0 1 7 7v10z"/></svg>`;
                    } else if (onclickStr.includes('roundrect')) {
                        btn.title = "مستطيل بزوايا دائرية";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="5" width="18" height="14" rx="4" ry="4"/></svg>`;
                    } else if (onclickStr.includes('rect') && !onclickStr.includes('open_left_rect')) {
                        btn.title = "مستطيل";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="3" y="6" width="18" height="12"/></svg>`;
                    } else if (onclickStr.includes('open_left_rect')) {
                        btn.title = "مستطيل مفتوح اليسار";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M 3 6 L 21 6 L 21 18 L 3 18"/></svg>`;
                    } else if (onclickStr.includes('circle')) {
                        btn.title = "دائرة";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><circle cx="12" cy="12" r="9"/></svg>`;
                    } else if (onclickStr.includes('shelf')) {
                        btn.title = "رف / لوح خشب";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><rect x="2" y="10" width="20" height="4"/></svg>`;
                    } else if (onclickStr.includes('corner_salon')) {
                        btn.title = "زاوية صالون / ركنة";
                        btn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><path d="M 3 3 L 21 3 L 21 21 L 9 21 L 3 15 Z"/><polygon points="13 3 21 3 21 11" fill="currentColor"/></svg>`;
                    }
                });

                const straightBtn = document.getElementById('sketch-mode-straight');
                if (straightBtn) {
                    straightBtn.title = "خط مستقيم";
                    straightBtn.innerHTML = `<svg viewBox="0 0 24 24" width="16" height="16" fill="none" stroke="currentColor" stroke-width="2.5"><line x1="3" y1="21" x2="21" y2="3"/></svg>`;
                }
                const textBtn = document.querySelector("#sketch-view button[onclick='addSketchTextPrompt()']");
                if (textBtn) {
                    textBtn.title = "إضافة نص / قياس";
                }

                if (sketchCanvas) {
                    sketchCanvas.removeEventListener('mousemove', updateSketchCanvasTooltip);
                    sketchCanvas.addEventListener('mousemove', updateSketchCanvasTooltip);
                }
            }, 50);
        }

        // --- SKETCH COLORS & STYLES ---
        function initSketchColors() {
            const container = document.getElementById('sketch-color-palette');
            if (!container) return;
            container.innerHTML = SKETCH_PREMIUM_COLORS.map(color => `
                <button type="button" onclick="setSketchCurrentColor('${color}')" 
                        class="w-7 h-7 rounded-full border border-gray-300 focus:outline-none transition transform hover:scale-110" 
                        style="background-color: ${color};" 
                        id="sketch-color-btn-${color.replace('#', '')}"></button>
            `).join('');
            
            setSketchCurrentColor('#000000');
        }

        function setSketchCurrentColor(color) {
            sketchCurrentColor = color;
            SKETCH_PREMIUM_COLORS.forEach(c => {
                const btn = document.getElementById(`sketch-color-btn-${c.replace('#', '')}`);
                if (btn) {
                    if (c === color) {
            ...
            if (selectedSketchShape) {
                selectedSketchShape.color = color;
                drawSketch();
            }
        }

        function setSketchCustomColor(color) {
            setSketchCurrentColor(color);
        }

        function setSketchStrokeWidth(width) {
            sketchCurrentStrokeWidth = parseInt(width);
            document.getElementById('sketch-stroke-val').textContent = `${width}px`;
            
            if (selectedSketchShape) {
                selectedSketchShape.strokeWidth = sketchCurrentStrokeWidth;
                drawSketch();
            }
        }
        ...
        function setSketchMode(mode) {
            if (mode === 'straight' && sketchCurrentMode === 'straight') {
                sketchCurrentMode = 'draw';
            } else {
                sketchCurrentMode = mode;
            }
            ...
            if (!selectedSketchShape) {
                ...
                document.getElementById('sketch-shape-editor-controls').classList.remove('hidden');
            }
            drawSketch();
        }
        ...
        function addSketchShape(type) {
            if (!sketchCanvas) return;
            const x = sketchCanvas.width / 2;
            const y = sketchCanvas.height / 2;
            
            let w = 100;
            let h = 100;
            if (type === 'rect' || type === 'roundrect' || type === 'roundrect_top' || type === 'open_left_rect') {
                w = 160;
                h = 90;
            } else if (type === 'shelf') {
                w = 200;
                h = 15;
            } else if (type === 'square' || type === 'circle' || type === 'cut_corner_square' || type === 'corner_salon') {
                w = 100;
                h = 100;
            }
            ...
            const newShape = {
                ...
                width: w,
                height: h,
                color: sketchCurrentColor,
                strokeWidth: sketchCurrentStrokeWidth,
                angle: 0,
                text: ''
            };
            
            sketchShapes.push(newShape);
            selectedSketchShape = newShape;
            setSketchMode('select');
            updateSketchShapeEditorUI();
            drawSketch();
        }

        function addSketchTextPrompt() {
            if (!sketchCanvas || !sketchCtx) return;
            const input = document.getElementById('sketch-text-prompt-input');
            if (input) {
                input.value = '';
                if (!input.dataset.listener) {
                    input.addEventListener('keypress', (e) => {
                        if (e.key === 'Enter') submitSketchTextPrompt();
                    });
                    input.dataset.listener = 'true';
                }
            }
            openModal('sketch-text-prompt-modal');
            setTimeout(() => {
                if (input) input.focus();
            }, 100);
        }

        function submitSketchTextPrompt() {
            const input = document.getElementById('sketch-text-prompt-input');
            if (!input) return;
            const txt = input.value.trim();
            if (txt === '') {
                closeModal('sketch-text-prompt-modal');
                return;
            }
            ...
            sketchCtx.font = 'bold 16px Cairo';
            const textWidth = sketchCtx.measureText(txt).width;
        ...
        window.submitSketchTextPrompt = submitSketchTextPrompt;

        function deleteSelectedSketch() {
            if (selectedSketchShape && !selectedSketchShape.isLocked) {
                sketchShapes = sketchShapes.filter(s => s.id !== selectedSketchShape.id);
                sketchFreehandLines = sketchFreehandLines.filter(s => s.id !== selectedSketchShape.id);
                selectedSketchShape = null;
                document.getElementById('sketch-shape-editor-controls').classList.add('hidden');
                drawSketch();
            }
        ...
        function rotateSelectedSketchShape(val) {
            if (selectedSketchShape && !selectedSketchShape.isLocked) {
                selectedSketchShape.angle = (parseInt(val) * Math.PI) / 180;
                document.getElementById('sketch-rotate-val').textContent = `${val}°`;
                drawSketch();
            }
        }

        function resizeSelectedSketch(scale) {
            if (selectedSketchShape && !selectedSketchShape.isLocked) {
                selectedSketchShape.width = Math.max(20, selectedSketchShape.width * scale);
                selectedSketchShape.height = Math.max(20, selectedSketchShape.height * scale);
                drawSketch();
            }
        }

        function toggleLockSelectedSketch() {
            if (selectedSketchShape) {
                selectedSketchShape.isLocked = !selectedSketchShape.isLocked;
                updateSketchShapeEditorUI();
                drawSketch();
            }
        }
        window.toggleLockSelectedSketch = toggleLockSelectedSketch;

        function duplicateSelectedSketch() {
            if (!selectedSketchShape) return;
            
            const newShape = {
                id: Date.now() + Math.random(),
                type: selectedSketchShape.type,
                x: selectedSketchShape.x + 20,
                y: selectedSketchShape.y + 20,
                width: selectedSketchShape.width,
                height: selectedSketchShape.height,
                color: selectedSketchShape.color,
                strokeWidth: selectedSketchShape.strokeWidth,
                angle: selectedSketchShape.angle,
                text: selectedSketchShape.text || '',
                isLocked: false
            };
            
            if (selectedSketchShape.points) {
                newShape.points = selectedSketchShape.points.map(pt => ({
                    x: pt.x + 20,
                    y: pt.y + 20
                }));
            }
        ...
        window.duplicateSelectedSketch = duplicateSelectedSketch;

        // Keyboard shortcuts for copy/paste/duplicate
        let copiedSketchShape = null;

        document.addEventListener('keydown', (e) => {
            if (currentView !== 'sketch') return;
            if (document.activeElement.tagName === 'INPUT' || document.activeElement.tagName === 'TEXTAREA') return;

            // Ctrl+C (Copy)
            if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'c') {
                if (selectedSketchShape) {
                    copiedSketchShape = JSON.parse(JSON.stringify(selectedSketchShape));
                    e.preventDefault();
                }
            }
            // Ctrl+V (Paste)
            else if ((e.ctrlKey || e.metaKey) && e.key.toLowerCase() === 'v') {
                if (copiedSketchShape) {
                    const newShape = JSON.parse(JSON.stringify(copiedSketchShape));
                    ...
                    // Update copied reference location to allow continuous pasting with offset
                    copiedSketchShape.x += 20;
                    copiedSketchShape.y += 20;
                    if (copiedSketchShape.points) {
                        copiedSketchShape.points.forEach(pt => {
                            pt.x += 20;
                            pt.y += 20;
                        });
                    }
            ...
            // Backspace or Delete (Delete Shape)
        ...
        function updateSketchShapeEditorUI() {
            const controls = document.getElementById('sketch-shape-editor-controls');
            if (!controls) return;
            if (selectedSketchShape) {
                controls.classList.remove('hidden');
                const angleDeg = Math.round((selectedSketchShape.angle * 180) / Math.PI) % 360;
                document.getElementById('sketch-rotate-slider').value = angleDeg < 0 ? angleDeg + 360 : angleDeg;
                document.getElementById('sketch-rotate-val').textContent = `${angleDeg}°`;
                document.getElementById('sketch-stroke-slider').value = selectedSketchShape.strokeWidth;
                document.getElementById('sketch-stroke-val').textContent = `${selectedSketchShape.strokeWidth}px`;
                
                // Lock button UI update
                const lockBtn = document.getElementById('sketch-lock-btn');
                if (lockBtn) {
                    if (selectedSketchShape.isLocked) {
                        lockBtn.innerHTML = `<i class="fas fa-unlock ml-1.5"></i> إلغاء قفل الشكل`;
                        lockBtn.className = 'w-full py-2 bg-green-600 hover:bg-green-700 text-white rounded-lg font-bold text-xs flex items-center justify-center';
                    } else {
                        lockBtn.innerHTML = `<i class="fas fa-lock ml-1.5"></i> قفل الشكل`;
                        lockBtn.className = 'w-full py-2 bg-blue-500 hover:bg-blue-600 text-white rounded-lg font-bold text-xs flex items-center justify-center';
                    }
                }

                // Disable sliders and resize buttons if shape is locked
                const rotateSlider = document.getElementById('sketch-rotate-slider');
                if (rotateSlider) rotateSlider.disabled = selectedSketchShape.isLocked;

                const strokeSlider = document.getElementById('sketch-stroke-slider');
                if (strokeSlider) strokeSlider.disabled = selectedSketchShape.isLocked;
                
                const deleteBtn = controls.querySelector('button[onclick="deleteSelectedSketch()"]');
                if (deleteBtn) {
                    if (selectedSketchShape.isLocked) {
                ...
                const resizeButtons = controls.querySelectorAll('button[onclick^="resizeSelectedSketch"]');
                resizeButtons.forEach(btn => {
                    btn.disabled = selectedSketchShape.isLocked;
                    if (selectedSketchShape.isLocked) {
        ...
        function drawSketch() {
            ...
            // 1. Draw Freehand Lines and Straight Lines
            sketchFreehandLines.forEach(line => {
                if (line.points.length < 2) return;
                sketchCtx.beginPath();
                sketchCtx.strokeStyle = line.color;
                sketchCtx.lineWidth = line.width;
                sketchCtx.lineCap = 'round';
                sketchCtx.lineJoin = 'round';
                
                sketchCtx.moveTo(line.points[0].x, line.points[0].y);
                if (line.type === 'straight_line') {
                    sketchCtx.lineTo(line.points[1].x, line.points[1].y);
                } else {
                    for (let i = 1; i < line.points.length; i++) {
                        sketchCtx.lineTo(line.points[i].x, line.points[i].y);
                    }
                }
                sketchCtx.stroke();
            });
            ...
            if (sketchCurrentLine && sketchCurrentLine.points.length >= 2) {
                sketchCtx.beginPath();
                sketchCtx.strokeStyle = sketchCurrentLine.color;
                sketchCtx.lineWidth = sketchCurrentLine.width;
                sketchCtx.lineCap = 'round';
                sketchCtx.lineJoin = 'round';
                sketchCtx.moveTo(sketchCurrentLine.points[0].x, sketchCurrentLine.points[0].y);
                if (sketchCurrentLine.type === 'straight_line') {
                    sketchCtx.lineTo(sketchCurrentLine.points[1].x, sketchCurrentLine.points[1].y);
                } else {
                    for (let i = 1; i < sketchCurrentLine.points.length; i++) {
                        sketchCtx.lineTo(sketchCurrentLine.points[i].x, sketchCurrentLine.points[i].y);
                    }
                }
                sketchCtx.stroke();
            }
            ...
            sketchShapes.forEach(shape => {
                sketchCtx.save();
                sketchCtx.translate(shape.x, shape.y);
                sketchCtx.rotate(shape.angle);
                
                sketchCtx.strokeStyle = shape.color;
                sketchCtx.lineWidth = shape.strokeWidth;
                sketchCtx.fillStyle = 'rgba(255, 255, 255, 0.2)'; 
                
                const w = shape.width;
                const h = shape.height;

                if (shape.type === 'square' || shape.type === 'rect' || shape.type === 'shelf') {
                    sketchCtx.beginPath();
                    sketchCtx.rect(-w/2, -h/2, w, h);
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'circle') {
                    sketchCtx.beginPath();
                    sketchCtx.arc(0, 0, w/2, 0, Math.PI * 2);
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'roundrect') {
                    const r = 15; 
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(-w/2 + r, -h/2);
                    sketchCtx.lineTo(w/2 - r, -h/2);
                    sketchCtx.quadraticCurveTo(w/2, -h/2, w/2, -h/2 + r);
                    sketchCtx.lineTo(w/2, h/2 - r);
                    sketchCtx.quadraticCurveTo(w/2, h/2, w/2 - r, h/2);
                    sketchCtx.lineTo(-w/2 + r, h/2);
                    sketchCtx.quadraticCurveTo(-w/2, h/2, -w/2, h/2 - r);
                    sketchCtx.lineTo(-w/2, -h/2 + r);
                    sketchCtx.quadraticCurveTo(-w/2, -h/2, -w/2 + r, -h/2);
                    ...
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'roundrect_top') {
                    const r = 15; 
                    sketchCtx.beginPath();
                    // Top-left to Top-right with rounding
                    sketchCtx.moveTo(-w/2 + r, -h/2);
                    sketchCtx.lineTo(w/2 - r, -h/2);
                    sketchCtx.quadraticCurveTo(w/2, -h/2, w/2, -h/2 + r);
                    // Top-right to Bottom-right (sharp corner)
                    sketchCtx.lineTo(w/2, h/2);
                    // Bottom-right to Bottom-left (sharp corner)
                    sketchCtx.lineTo(-w/2, h/2);
                    // Bottom-left to Top-left with rounding
                    sketchCtx.lineTo(-w/2, -h/2 + r);
                    sketchCtx.quadraticCurveTo(-w/2, -h/2, -w/2 + r, -h/2);
                    sketchCtx.closePath();
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'cut_corner_square') {
                    const cut = Math.min(w, h) * 0.2;
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(-w/2 + cut, -h/2);
                    sketchCtx.lineTo(w/2, -h/2);
                    sketchCtx.lineTo(w/2, h/2);
                    sketchCtx.lineTo(-w/2, h/2);
                    sketchCtx.lineTo(-w/2, -h/2 + cut);
                    sketchCtx.closePath();
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'corner_salon') {
                    const cut = Math.min(w, h) * 0.25;
                    const offset = Math.min(w, h) * 0.55;
                    
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(-w/2, -h/2);
                    sketchCtx.lineTo(w/2, -h/2);
                    sketchCtx.lineTo(w/2, h/2);
                    sketchCtx.lineTo(-w/2 + cut, h/2);
                    sketchCtx.lineTo(-w/2, h/2 - cut);
                    sketchCtx.closePath();
                    sketchCtx.fill();
                    sketchCtx.stroke();
                    
                    // Draw and fill corner triangle with black
                    sketchCtx.fillStyle = '#000000';
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(w/2 - offset, -h/2);
                    sketchCtx.lineTo(w/2, -h/2);
                    sketchCtx.lineTo(w/2, -h/2 + offset);
                    sketchCtx.closePath();
                    sketchCtx.fill();
                    sketchCtx.stroke();
                } else if (shape.type === 'open_left_rect') {
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(-w/2, -h/2);
                    sketchCtx.lineTo(w/2, -h/2);
                    sketchCtx.lineTo(w/2, h/2);
                    ...
                    sketchCtx.beginPath();
                    sketchCtx.moveTo(-w/2, 0);
                    sketchCtx.lineTo(w/2, 0);
                    sketchCtx.stroke();
                } else if (shape.type === 'text') {
                    sketchCtx.fillStyle = shape.color;
                    sketchCtx.font = 'bold 16px Cairo';
                    sketchCtx.textAlign = 'center';
                    sketchCtx.textBaseline = 'middle';
                    sketchCtx.fillText(shape.text, 0, 0);
                }

                sketchCtx.restore();
            });

            // 3. Draw bounding box and transform handles if selecting
            if (selectedSketchShape) {
                drawSketchSelectionOverlay(selectedSketchShape);
            }
        }

        function drawSketchSelectionOverlay(shape) {
            if (!sketchCtx) return;
            sketchCtx.save();
            ...
            if (shape.points) {
                const bbox = getLineBoundingBox(shape);
                x = bbox.x;
                y = bbox.y;
                w = bbox.width;
                h = bbox.height;
            ...
            sketchCtx.rotate(angle);

            // Dashed selection box (red if locked, orange/amber if unlocked)
            sketchCtx.strokeStyle = shape.isLocked ? '#ef4444' : '#f59e0b';
            sketchCtx.lineWidth = 1;
            sketchCtx.setLineDash([5, 3]);
            sketchCtx.strokeRect(-w/2 - 5, -h/2 - 5, w + 10, h + 10);
            sketchCtx.setLineDash([]);

            if (!shape.points && !shape.isLocked) {
                // Resize anchor handle (bottom-right)
                sketchCtx.fillStyle = '#f59e0b';
                sketchCtx.fillRect(w/2, h/2, 10, 10);

                // Rotation handle (top line + pivot point)
                sketchCtx.beginPath();
                sketchCtx.moveTo(0, -h/2 - 5);
                sketchCtx.lineTo(0, -h/2 - 25);
                sketchCtx.strokeStyle = '#f59e0b';
                sketchCtx.stroke();

                sketchCtx.beginPath();
                sketchCtx.arc(0, -h/2 - 25, 6, 0, Math.PI*2);
                sketchCtx.fillStyle = '#3b82f6';
                sketchCtx.fill();
                sketchCtx.stroke();
            } else if (shape.isLocked) {
                // Draw a small lock emoji indicator
                sketchCtx.fillStyle = '#ef4444';
                sketchCtx.font = '12px Arial';
                sketchCtx.fillText('🔒', -w/2 - 12, -h/2 - 12);
            }

            sketchCtx.restore();
        }

        // --- MATH INTERACTION HELPERS ---
        function getLineBoundingBox(line) {
        ...
        function isPointNearLine(p, line) {
            if (!line.points || line.points.length < 2) return false;
            for (let i = 0; i < line.points.length - 1; i++) {
                const p1 = line.points[i];
                const p2 = line.points[i+1];
                if (getDistanceToSegment(p, p1, p2) < 15) {
                    return true;
                }
            }
            return false;
        ...
        function isPointInSketchShape(p, shape) {
            ...
            if (shape.type === 'circle') {
                return (rx * rx) + (ry * ry) <= (shape.width / 2) * (shape.width / 2);
            } else {
                return Math.abs(rx) <= shape.width / 2 && Math.abs(ry) <= shape.height / 2;
            }
        }

        function getSketchHandleAtPoint(p, shape) {
            const dx = p.x - shape.x;
            const dy = p.y - shape.y;
            const cos = Math.cos(-shape.angle);
            const sin = Math.sin(-shape.angle);
            ...
            // Rotation handle
            const distRot = Math.hypot(rx - 0, ry - (-h/2 - 25));
            if (distRot <= 10) return 'rotate';

            // Resize handle (bottom right)
            const distResize = Math.hypot(rx - w/2, ry - h/2);
            if (distResize <= 12) return 'resize';

            return null;
        }

        // --- SKETCH EVENT INTERCEPTIONS ---
        function getSketchCanvasCoords(e) {
        ...
        function onSketchPointerStart(e) {
            ...
            if (selectedSketchShape && !selectedSketchShape.isLocked) {
                ...
                if (handle === 'rotate') {
                    isSketchRotating = true;
                    sketchRotateCenter = { x: selectedSketchShape.x, y: selectedSketchShape.y };
                    const dx = p.x - sketchRotateCenter.x;
                    const dy = p.y - sketchRotateCenter.y;
                    sketchStartAngle = Math.atan2(dy, dx) - selectedSketchShape.angle;
                    e.preventDefault();
                    return;
                } 
                else if (handle === 'resize') {
                    isSketchResizing = true;
                    e.preventDefault();
                    return;
                }
            }

            // 2. Check if we clicked on any shape to select and drag it (only if in select mode)
            let found = null;
            if (sketchCurrentMode === 'select') {
                for (let i = sketchShapes.length - 1; i >= 0; i--) {
                    if (isPointInSketchShape(p, sketchShapes[i])) {
                        found = sketchShapes[i];
                        break;
                ...
                if (!found) {
                    for (let i = sketchFreehandLines.length - 1; i >= 0; i--) {
                        if (isPointNearLine(p, sketchFreehandLines[i])) {
                            found = sketchFreehandLines[i];
                            if (!found.id) found.id = Date.now() + Math.random();
                            const bbox = getLineBoundingBox(found);
                            found.x = bbox.x;
                            found.y = bbox.y;
                            found.width = bbox.width;
                            found.height = bbox.height;
                            break;
                        }
                    }
                }
            ...
            if (found) {
                selectedSketchShape = found;
                if (!selectedSketchShape.isLocked) {
                    isSketchDragging = true;
                    sketchDragOffsetX = p.x - selectedSketchShape.x;
                    sketchDragOffsetY = p.y - selectedSketchShape.y;
                } else {
                    isSketchDragging = false;
                }
                updateSketchShapeEditorUI();
                ...
                // Clicked on empty space: clear shape selection
                selectedSketchShape = null;
                updateSketchShapeEditorUI();

                // Only start drawing if mode is 'straight'
                if (sketchCurrentMode === 'straight') {
                    isSketchDrawing = true;
                    sketchCurrentLine = {
                        type: 'straight_line',
                        points: [p, p],
                        color: sketchCurrentColor,
                        width: sketchCurrentStrokeWidth
                    };
                    drawSketch();
                }
            }
        ...
        function onSketchPointerMove(e) {
            ...
            if (isSketchDrawing && sketchCurrentLine) {
                if (sketchCurrentLine.type === 'straight_line') {
                    sketchCurrentLine.points[1] = p;
                } else {
                    sketchCurrentLine.points.push(p);
                }
                drawSketch();
            } 
            else if (isSketchDragging && selectedSketchShape) {
                if (selectedSketchShape.points) {
                    const dx = p.x - sketchDragOffsetX - selectedSketchShape.x;
                    const dy = p.y - sketchDragOffsetY - selectedSketchShape.y;
                    selectedSketchShape.points.forEach(pt => {
                        pt.x += dx;
                        pt.y += dy;
                    });
                    const bbox = getLineBoundingBox(selectedSketchShape);
                    selectedSketchShape.x = bbox.x;
                    selectedSketchShape.y = bbox.y;
                } else {
                    selectedSketchShape.x = p.x - sketchDragOffsetX;
                    selectedSketchShape.y = p.y - sketchDragOffsetY;
                }
                drawSketch();
            } 
            else if (isSketchRotating && selectedSketchShape) {
                const dx = p.x - sketchRotateCenter.x;
                const dy = p.y - sketchRotateCenter.y;
                const currentAngle = Math.atan2(dy, dx);
                selectedSketchShape.angle = currentAngle - sketchStartAngle;
                
                let angleDeg = Math.round((selectedSketchShape.angle * 180) / Math.PI) % 360;
                if (angleDeg < 0) angleDeg += 360;
                document.getElementById('sketch-rotate-slider').value = angleDeg;
                document.getElementById('sketch-rotate-val').textContent = `${angleDeg}°`;
                drawSketch();
            } 
            else if (isSketchResizing && selectedSketchShape) {
                const dx = p.x - selectedSketchShape.x;
                const dy = p.y - selectedSketchShape.y;
                
                const cos = Math.cos(-selectedSketchShape.angle);
                const sin = Math.sin(-selectedSketchShape.angle);
                const rx = dx * cos - dy * sin;
                const ry = dx * sin + dy * cos;

                selectedSketchShape.width = Math.max(20, Math.abs(rx) * 2);
                selectedSketchShape.height = Math.max(20, Math.abs(ry) * 2);
                drawSketch();
            }
        }

        function onSketchPointerEnd(e) {
            if (isSketchDrawing && sketchCurrentLine) {
                if (sketchCurrentLine.type === 'straight_line') {
                    const p1 = sketchCurrentLine.points[0];
                    const p2 = sketchCurrentLine.points[1];
                    const dx = p2.x - p1.x;
                    const dy = p2.y - p1.y;
                    ...
                    if (len > 5) {
                        ...
                        const newLine = {
                            ...
                            height: 15,
                            angle: angle,
                            color: sketchCurrentLine.color,
                            strokeWidth: sketchCurrentLine.width
                        };
                        sketchShapes.push(newLine);
                    }
                } else {
                    sketchFreehandLines.push(sketchCurrentLine);
                }
                sketchCurrentLine = null;
            }
            isSketchDrawing = false;
        ...
        // --- GLOBAL ACTIONS ---
        function clearSketchCanvasPrompt() {
            showCustomConfirm(
                'مسح اللوحة',
                'هل أنت متأكد من مسح جميع الرسومات والأشكال؟ لا يمكن التراجع عن هذا الإجراء.',
                () => {
                    sketchShapes = [];
                    sketchFreehandLines = [];
                    selectedSketchShape = null;
        ...
        function saveSketchToServer() {
            if (!sketchInvoiceNo) {
                showAlert('رقم الفاتورة مفقود.', 'error');
                return;
            }
            const data = {
                shapes: sketchShapes,
                freehandLines: sketchFreehandLines
            };
            // حفظ محلي فوري
            saveToLocal(`sketch_data_${sketchInvoiceNo}`, data);
            
            // مزامنة مع السيرفر كحقل إضافي في الفاتورة أو تخزين مستقل
            // نقوم بإرسال التعديل للسيرفر لحفظ كروكي صالون
            scriptRunHelper('saveInvoiceSketch', [sketchInvoiceNo, JSON.stringify(data)], (result) => {
                if (result && result.success) {
                    showAlert('تم حفظ الكروكي بنجاح على السيرفر.', 'success');
                } else {
                    showAlert('تم الحفظ محلياً وجاري المزامنة في الخلفية...', 'info');
                }
            });
        }

        function loadSketchFromServer(silent = true) {
            if (!sketchInvoiceNo) return;
            
            // قراءة من السيرفر أولاً لمزامنة التغييرات
            scriptRunHelper('getInvoiceSketch', [sketchInvoiceNo], (result) => {
                if (result && result.success && result.sketchData) {
                    try {
                        const sData = JSON.parse(result.sketchData);
                        sketchShapes = sData.shapes || [];
                        sketchFreehandLines = sData.freehandLines || [];
                        selectedSketchShape = null;
                        updateSketchShapeEditorUI();
                        drawSketch();
                        saveToLocal(`sketch_data_${sketchInvoiceNo}`, sData);
                        if (!silent) showAlert('تم تحميل الكروكي من السيرفر.', 'success');
                    } catch (e) {
                        if (!silent) showAlert('حدث خطأ أثناء قراءة الكروكي.', 'error');
                    }
                } else {
                    // قراءة محلية كاحتياط
                    const localData = getFromLocal(`sketch_data_${sketchInvoiceNo}`);
                    if (localData) {
                        sketchShapes = localData.shapes || [];
                        sketchFreehandLines = localData.freehandLines || [];
                        selectedSketchShape = null;
                        updateSketchShapeEditorUI();
                        drawSketch();
                        if (!silent) showAlert('تم استعادة الكروكي المحفوظ بنجاح.', 'success');
                    } else {
                        if (!silent) showAlert('لا يوجد كروكي محفوظ لهذا الزبون.', 'info');
                    }
                }
            });
        }
        ...
        function exportSketchToPDF() {
            generatePrintableSketch(sketchInvoiceNo);
        }
        
        function toggleSketchTools() {
            const sidebar = document.getElementById('sketch-sidebar');
            if (sidebar) {
                sidebar.classList.toggle('hidden');
            }
        }
        window.toggleSketchTools = toggleSketchTools;
        ...
        function showView(viewId) {
             ...
             // Collapse main app sidebar to make designing easier
             ...
             if (viewId === 'sketch') {
                 ...
                 if (mainContentEl) {
                     mainContentEl.style.marginRight = '';
        ...
        function generatePrintableInvoice(invoiceNo) {
            ...
            // التشغيل المباشر لطباعة الفاتورة عبر نافذة الطباعة الافتراضية للجهاز
            setTimeout(() => {
                window.print();
            }, 300);
        }
        ...
        function handleElectricitySubmit(event) {
            ...
            const submitButton = document.getElementById('electricity-submit-button');
        ...
        // ====================================================================
        // NEW: وظائف تجميع الكروكي ورسمه على كانفاس موحد وتصديره مباشرة
        // ====================================================================
        function drawSketchOnContext(ctx, width, height, data) {
            ctx.clearRect(0, 0, width, height);
            
            // خلفية بيضاء لمنطقة الرسم
            ctx.fillStyle = '#ffffff';
            ctx.fillRect(0, 0, width, height);
            
            const scaleX = width / 734;
            ...
            const safeData = data || {};
            const freehandLines = safeData.freehandLines || [];
            const shapes = safeData.shapes || [];

            // 1. رسم الخطوط الحرة والخطوط المستقيمة
            freehandLines.forEach(line => {
                ...
                if (line.type === 'straight_line') {
                    ctx.lineTo(line.points[1].x * scaleX, line.points[1].y * scaleY);
                } else {
                    for (let i = 1; i < line.points.length; i++) {
            ...
            // 2. رسم الأشكال الهندسية والنصوص
            shapes.forEach(shape => {
                ctx.save();
                ctx.translate(shape.x * scaleX, shape.y * scaleY);
        ...
        function compileSketchToCanvas(invoice, sketchData, callback) {
            let canvas = document.createElement('canvas');
            canvas.width = 800;
            canvas.height = 1100;
            let ctx = canvas.getContext('2d');

            // جلب شعار المتجر من خلال الـ alt نظراً لتشفيره بصيغة Base64
            const logoImg = document.querySelector('img[alt*="Logo"]');
            const isLocal = window.location.protocol === 'file:';
            const canDrawLogo = logoImg && logoImg.complete && logoImg.naturalWidth > 0 && 
                                (logoImg.src.startsWith('data:image/') || !isLocal);
            
            const drawHeaderAndContent = (drawLogo = true) => {
                if (!drawLogo) {
                    // Create a fresh canvas to prevent taint carry-over
                ...
                // تعبئة الخلفية باللون الأبيض للبدء النظيف
                ctx.fillStyle = '#ffffff';
                ctx.fillRect(0, 0, canvas.width, canvas.height);

                // 1. رسم شعار المتجر عن اليمين (إذا كان مفعل ولم يسبب خطأ أمني سابق)
                if (drawLogo && logoImg && logoImg.complete && logoImg.naturalWidth > 0) {
                    try {
                        ctx.drawImage(logoImg, 664, 30, 96, 96);
                    } catch (e) {
                        console.warn("Failed to draw logo on canvas: ", e);
                    }
                }

                // 2. رسم النصوص التعريفية للمتجر (بجانب الشعار عن اليمين)
                ctx.fillStyle = '#000000';
                ctx.font = 'bold 24px Cairo, Arial, sans-serif';
                ctx.textAlign = 'right';
                ctx.textBaseline = 'top';
                ctx.fillText('موبل أرت ديزاين', 650, 35);
                
                ctx.font = '14px Cairo, Arial, sans-serif';
                ctx.fillStyle = '#4b5563';
                ctx.fillText('حي النهضة رقم 135 بلوك M', 650, 70);
                ctx.fillText('الهاتف: 0640442778', 650, 95);

                // 3. رسم صندوق معلومات الزبون عن اليسار
                ctx.strokeStyle = '#d1d5db';
                ctx.lineWidth = 1;
                const rX = 40, rY = 30, rW = 340, rH = 100, radius = 8;
                ctx.beginPath();
                ...
                const options = { year: 'numeric', month: 'long', day: 'numeric' };
                const dateStr = new Date().toLocaleDateString('ar-MA', options);

                ctx.fillText(`الزبون: ${customerName}`, rX + rW - 15, rY + 15);
                ctx.fillText(`الهاتف: ${customerPhone}`, rX + rW - 15, rY + 38);
                ctx.fillText(`رقم الفاتورة: ${invoiceNo}`, rX + rW - 15, rY + 61);
                
                ctx.fillStyle = '#9ca3af';
                ctx.font = '11px Cairo, Arial, sans-serif';
                ctx.fillText(`التاريخ: ${dateStr}`, rX + rW - 15, rY + 82);

                // 4. خط الفصل الأصفر الجميل
                ctx.fillStyle = '#f59e0b';
                ctx.fillRect(40, 150, 720, 4);

                // 5. إنشاء كانفاس كروكي مؤقت لرسم الأشكال بالقياس المناسب
                const tempSketchCanvas = document.createElement('canvas');
                tempSketchCanvas.width = 720;
                tempSketchCanvas.height = 880; // Increased height to occupy the remaining space
                const tempSketchCtx = tempSketchCanvas.getContext('2d');
                
                drawSketchOnContext(tempSketchCtx, tempSketchCanvas.width, tempSketchCanvas.height, sketchData);
                
                // رسم الكروكي في الكانفاس الموحد الرئيسي
                ctx.drawImage(tempSketchCanvas, 40, 170, 720, 880);

                // التحقق من تلوث الكانفاس عند محاولة استخراج الصورة لتفادي أخطاء CORS الأمنية
                try {
                    callback(canvas.toDataURL('image/png'));
                } catch (err) {
                    if (drawLogo) {
                        console.warn("Canvas tainted by logo. Re-rendering without logo to prevent SecurityError.");
                        drawHeaderAndContent(false);
                    } else {
                        console.error("Failed to export canvas even without logo: ", err);
                        try {
                            callback(canvas.toDataURL('image/png'));
                        } catch (finalErr) {
                            console.error("Final export failure: ", finalErr);
                        }
                    }
                }
            };

            drawHeaderAndContent(canDrawLogo);
        }

        function exportSketchPDFFromData(invoice, sketchData, filename) {
            showAlert('جاري تجهيز الكروكي بصيغة PDF...', 'info');
            compileSketchToCanvas(invoice, sketchData, (dataUrl) => {
                // المحاولة الأولى: التصدير المباشر والسريع عبر jsPDF لتفادي أي مشاكل في html2canvas
                let jsPDFClass = null;
                if (window.jspdf && window.jspdf.jsPDF) {
                    jsPDFClass = window.jspdf.jsPDF;
                } else if (window.jsPDF) {
                    jsPDFClass = window.jsPDF;
                } else if (typeof html2pdf !== 'undefined' && html2pdf.Worker && html2pdf.Worker.prototype && html2pdf.Worker.prototype.jsPDF) {
                    jsPDFClass = html2pdf.Worker.prototype.jsPDF;
                }
                ...
                if (jsPDFClass) {
                    try {
                        ...
                        // مقاسات صفحة A4 هي 210 مم × 297 مم
                        doc.addImage(dataUrl, 'PNG', 0, 0, 210, 297);
                        doc.save(filename);
                        showAlert('تم تحميل الكروكي بنجاح.', 'success');
                        return;
                    } catch (e) {
                        console.warn("Direct jsPDF export failed, falling back to html2pdf...", e);
                    }
                }

                // طريقة احتياطية في حال لم تتوفر فئة jsPDF بشكل مستقل
                ...
                html2pdf().set(opt).from(img).save().then(() => {
                    document.body.removeChild(img);
                    showAlert('تم تحميل الكروكي بنجاح.', 'success');
                }).catch(err => {
                    document.body.removeChild(img);
                    console.error("PDF generation error: ", err);
                    showAlert('خطأ أثناء تحميل PDF الكروكي.', 'error');
                });
            });
        }

        function downloadSketchPDFDirectly(invoiceNo) {
            const invoice = allInvoices.find(inv => String(inv.InvoiceNo).trim() === String(invoiceNo).trim());
            if (!invoice) {
                showAlert('لم يتم العثور على الفاتورة.', 'error');
                return;
            ...
            let sketchData = getFromLocal(`sketch_data_${invoiceNo}`);
            if (!sketchData && invoice.SketchData) {
                try {
                    sketchData = JSON.parse(invoice.SketchData);
                } catch (e) {
                    console.error("Error parsing sketch data", e);
                }
            }
            
            const cleanInvoiceNo = String(invoiceNo).replace(/[^a-zA-Z0-9]/g, '_');
            const cleanCustomerName = String(invoice.CustomerName || 'customer').replace(/\s+/g, '_');
            const filename = `Sketch_${cleanCustomerName}_Facture_${cleanInvoiceNo}.pdf`;

            if (!sketchData) {
                showAlert('جاري جلب بيانات الكروكي من السيرفر...', 'info');
                scriptRunHelper('getInvoiceSketch', [invoiceNo], (result) => {
                    if (result && result.success && result.sketchData) {
                        try {
                            const sData = JSON.parse(result.sketchData);
                            saveToLocal(`sketch_data_${invoiceNo}`, sData);
                            exportSketchPDFFromData(invoice, sData, filename);
                        } catch (e) {
                            showAlert('حدث خطأ أثناء قراءة الكروكي من السيرفر.', 'error');
                        }
                    } else {
                        showAlert('لا يوجد كروكي مسجل لهذه الفاتورة بعد.', 'warning');
                    }
                });
            } else {
                exportSketchPDFFromData(invoice, sketchData, filename);
            }
        }
        window.downloadSketchPDFDirectly = downloadSketchPDFDirectly;

        function generatePrintableSketch(invoiceNo) {
            const invoice = allInvoices.find(inv => String(inv.InvoiceNo).trim() === String(invoiceNo).trim());
            if (!invoice) {
                showAlert('لم يتم العثور على الفاتورة.', 'error');
                return;
            ...
            const startPrinting = (sData) => {
                document.getElementById('print-sketch-customer-name').textContent = invoice.CustomerName || '-';
                document.getElementById('print-sketch-customer-phone').textContent = invoice.CustomerPhone || '-';
                document.getElementById('print-sketch-invoice-no').textContent = invoice.InvoiceNo || '-';
                
                let dateText = '-';
                if (invoice.Date) {
                    try {
                        dateText = formatDateDMY(invoice.Date);
                    } catch (e) {
                        console.error(e);
                    }
                }
                document.getElementById('print-sketch-date').textContent = dateText;
                
                const logoImg = document.querySelector('img[alt*="Logo"]');
                const printLogoImg = document.getElementById('print-sketch-logo');
                if (logoImg && printLogoImg) {
                    printLogoImg.src = logoImg.src;
                }

                const printCanvas = document.getElementById('print-sketch-canvas');
                if (printCanvas) {
                    printCanvas.width = 734;
                    printCanvas.height = 900; // Increased height to fill the bottom of the page
                    const ctx = printCanvas.getContext('2d');
                    drawSketchOnContext(ctx, printCanvas.width, printCanvas.height, sData);
                    
                    // Convert canvas to image for Safari & tablet printing compatibility
                    let printImg = document.getElementById('print-sketch-img');
                    if (!printImg) {
                        printImg = document.createElement('img');
                        printImg.id = 'print-sketch-img';
                        printImg.style.maxWidth = '100%';
                        printImg.style.height = 'auto';
                        printImg.style.display = 'block';
                        printImg.style.border = '1px solid #d1d5db';
                        printCanvas.parentNode.appendChild(printImg);
                    }
                ...
                openModal('printable-sketch-modal');
                
                const handleAfterPrint = () => {
                    closeModal('printable-sketch-modal');
            ...
            if (!sketchData) {
                ...
                startPrinting(sketchData);
            }
        }
        window.generatePrintableSketch = generatePrintableSketch;
