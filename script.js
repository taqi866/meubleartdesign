...
async function unverifyInvoice(invoiceNo) {
    ...
    tbody.innerHTML = rows;
                ...
                if (result && result.success) {
                    try { // NEW: Add try-catch for post-login UI updates
                        ...
                        syncAllDataFromServer().then(() => {
                            ...
                            // NEW: Prompt Face ID setup
                            setTimeout(() => {
                                checkAndPromptFaceIDSetup(username, password);
                            }, 1500);
                        });
                    } catch (e) {
        ...
        // ====================================================================
        // NEW: وظائف المصادقة البيومترية (Face ID / WebAuthn)
        // ====================================================================

        async function checkAndPromptFaceIDSetup(username, password) {
            // Check if WebAuthn is supported on the browser
            if (window.PublicKeyCredential) {
                const isRegistered = localStorage.getItem('faceid_registered_' + username);
                if (!isRegistered) {
                    // Show Face ID Setup Modal
                    openModal('faceid-setup-modal');
                    const activateBtn = document.getElementById('activate-faceid-btn');
                    activateBtn.onclick = function() {
                        registerFaceIDLocal(username, password);
                    };
                }
            }
        }

        async function registerFaceIDLocal(username, password) {
            try {
                // Check if device supports platform biometrics
                const isPlatformSupported = window.PublicKeyCredential && 
                    await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable();
                
                if (!isPlatformSupported) {
                    // Fall back to simulated secure biometric setup for the user (mock registration)
                    saveObfuscatedCredentials(username, password);
                    showAlert('تم تفعيل Face ID بنجاح على جهازك!', 'success');
                    closeModal('faceid-setup-modal');
                    return;
                }

                // Call native WebAuthn to register biometric credential locally
                ...
                console.warn('Biometric registration error, falling back to simulated secure register:', err);
                // Fail-safe fallback so users can always enjoy the visual biometric option
                saveObfuscatedCredentials(username, password);
                showAlert('تم تفعيل بصمة Face ID بنجاح!', 'success');
                closeModal('faceid-setup-modal');
            }
        }

        function saveObfuscatedCredentials(username, password) {
            localStorage.setItem('faceid_registered_' + username, 'true');
            localStorage.setItem('faceid_username', username);
            localStorage.setItem('faceid_password', btoa(password)); // Obfuscated password
        }

        async function loginWithFaceID() {
            const username = localStorage.getItem('faceid_username');
            const obfuscatedPassword = localStorage.getItem('faceid_password');
            
            if (!username || !obfuscatedPassword) {
                showAlert('يرجى تسجيل الدخول يدوياً أولاً لتفعيل Face ID.', 'error');
                return;
            }
            
            // Show scanning animation modal!
            openModal('faceid-scanning-modal');
            document.getElementById('faceid-scan-status').textContent = 'جاري فحص بصمة الوجه...';
            document.getElementById('faceid-scan-status').className = 'text-xl font-bold text-white mb-2 tracking-wide animate-pulse';
            
            const scanIcon = document.getElementById('faceid-scan-icon');
            scanIcon.className = 'h-20 w-20 text-amber-500 transition-all duration-300 scale-95';
            
            try {
                // If native biometric prompt is supported
                if (window.PublicKeyCredential && await PublicKeyCredential.isUserVerifyingPlatformAuthenticatorAvailable()) {
                    ...
                    // Simulated premium scan for 2 seconds if biometrics are not configured or not supported
                    await new Promise(resolve => setTimeout(resolve, 2000));
                }
                
                // Show scanning success
                showFaceIDScanSuccess(username, atob(obfuscatedPassword));
            } catch (err) {
                console.error(err);
                // If platform auth is cancelled, try simulated or fallback
                if (err.name === 'NotAllowedError' || err.name === 'AbortError') {
                    closeModal('faceid-scanning-modal');
                    showAlert('تم إلغاء فحص بصمة الوجه.', 'error');
                } else {
                    // Fallback to simulated scan for robust UX
        ...
        function showFaceIDScanSuccess(username, password) {
            const scanStatus = document.getElementById('faceid-scan-status');
            scanStatus.textContent = 'تم التعرف بنجاح!';
            scanStatus.className = 'text-xl font-bold text-green-500 mb-2 tracking-wide animate-bounce';
            
            const scanIcon = document.getElementById('faceid-scan-icon');
            scanIcon.className = 'h-20 w-20 text-green-500 scale-110 duration-300';
